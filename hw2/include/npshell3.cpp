...
bool Npshell::np_using_fd(int fd) {
    ...
    if (shm_numpipe_list_pointer) {
        for (int i = 0; i < 30; i++) {
            if (FD_ISSET(fd, shm_numpipe_list_pointer + i))
                return true;
        }
    }
    if (user_list) {
    ...
    for (int i = 0; i < cmds.size(); i++) {
        ...
        for (int j = 0; j < numpipe_list.size(); j++) {
            if (numpipe_list[j].cnt <= 0) {
                ...
                FD_CLR(numpipe_list[j].read_end, shm_numpipe_list_pointer + (id - 1));
                FD_CLR(numpipe_list[j].write_end, shm_numpipe_list_pointer + (id - 1));
        ...
        while (1) {
            ...
            } else // parent process
            {
                ...
                if (is_numpipe && !is_write_to_numpipe) {
                    ...
                    FD_SET(pipe_fd[NP_PIPE_READ_END], shm_numpipe_list_pointer + (id - 1));
                    FD_SET(pipe_fd[NP_PIPE_WRITE_END], shm_numpipe_list_pointer + (id - 1));
                }
...
void Npshell::np_tell(int tid, string msg) {
    ...
    {
        strcpy(shm_msg_pointer, string("*** " + fnow -> name + " told you ***: " + msg + "\n").c_str());
        kill(tnow -> pid, SIGUSR1);
...
void Npshell::np_yell(string msg) {
    ...
    strcpy(shm_msg_pointer, string("*** " + now -> name + " yelled ***: " + msg + "\n").c_str());
    for (vector < User > ::iterator it = user_list -> begin(); it != user_list -> end(); it++)
...
void Npshell::np_name(string name) {
    ...
    if (check == user_list -> end()) // not found same name
    {
        now -> name = name;
        set_shm_user_list();
        strcpy(shm_msg_pointer, string("*** User from " + now -> addr + ":" + to_string(now -> port) + " is named '" + now -> name + "'. ***\n").c_str());
        for (vector < User > ::iterator it = user_list -> begin(); it != user_list -> end(); it++)
...
void Npshell::get_shm_user_list() {
    ...
    for (int i = 0; i < ( * shm_user_list_size_pointer); i++)
        user_list -> push_back(User(
            ...
            shm_user_list_pointer[i].pid)); //, cout<<shm_user_list_pointer[i].id<<endl;
}

void Npshell::set_shm_user_list() {
    if (!shm_user_list_pointer)
        return;
