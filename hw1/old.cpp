        ...
        for (int i = 0; i < cmds.size(); i++)
        {
            ...
            while (1)
            {
                ...
                { // child process
                    ...
                    if (is_pipe)
                    { // called pipe() this round, so dup the write end of
                        // pipe to stdout
                    ...
                    { // not called pipe(), but will write
                        // to numpipe
...
void np_clean_fd(pid_t pid)
{
    for (int i = 0; i < proc_list.size(); i++)
    {
        if (proc_list[i].pid == pid)
        {
            ...
            bool rfound = (proc_list[i].read_end == prev_read_end || proc_list[i].read_end == pipe_fd[PIPE_READ_END] || proc_list[i].read_end == pipe_fd[PIPE_WRITE_END]);
            bool wfound = (proc_list[i].write_end == prev_read_end || proc_list[i].write_end == pipe_fd[PIPE_READ_END] || proc_list[i].write_end == pipe_fd[PIPE_WRITE_END]);

            // should not close fd waiting in the numpipe list
            for (int j = 0; j < numpipe_list.size(); j++)
            {
                if (proc_list[i].read_end == numpipe_list[j].read_end || proc_list[i].read_end == numpipe_list[j].write_end)
                    rfound = true;
                if (proc_list[i].write_end == numpipe_list[j].read_end || proc_list[i].write_end == numpipe_list[j].write_end)
                    wfound = true;
            }
